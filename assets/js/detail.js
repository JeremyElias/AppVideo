...
const movieId= window.localStorage.getItem("movieId"); // id de la pelicula a mostrar
...
const filterVideos = function (videoList){
    return videoList.filter(({type}) => type === "Trailer" || type === "Teaser");
...
fetchDataFromServer(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${api_key}&append_to_response=casts,videos,images,releases`,function(movie){
    ...
    document.title = `${title} - iFliz`;

    const movieDetail = document.createElement("div");
    movieDetail.classList.add("movie-detail");
    movieDetail.innerHTML = `
    <div class="backdrop-image"
    style="background-image: url('${imageBaseURL}${"w1280"|| "original"}${backdrop_path || poster_path}');"></div>
<figure class="poster-box movie-poster">
    <img src="${imageBaseURL}w342${poster_path}"
        alt="${title} poster" class="img-cover">
</figure>
...
const addSuggestedMovies = function({results: movieList},title ){
    ...
    <div class="title-wrapper">
    <h3 class="title-large">También te puede interesar</h3>
</div>
